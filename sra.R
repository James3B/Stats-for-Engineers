...
areametric.mc = function(x,y) sum(abs(sort(x@x) - sort(y@x))) / MC$many
...
plot.mc <- function(x, t=NULL, xlab=t, ylab='Cumulative probability', cumulative=MC$cumulative, ...) {
  ...
  if (.Platform$OS.type=='windows') bringToTop(-1) # return focus to R console
}

edf.numeric <- function(x, new=TRUE, xlab='', ylab='Cumulative probability', xlim=range(x), ylim=c(0,1), ...) {
  if (new) plot(NULL,NULL,xlab=xlab,ylab=ylab,xlim=xlim,ylim=ylim, ...)
  lines(c(min(x),sort(x)),seq(0,1,length.out=1+length(x)),type='s', ...)
}

edf.mc <- function(b,xlab=deparse(substitute(b)),new=FALSE,...) edf.numeric(b@x,xlab=xlab,new=new,...)
...
tan        = tan = function(x,lwd=3,col='tan',...)        edf(x,lwd=lwd,col=col,...)
